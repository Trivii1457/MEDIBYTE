...
{
    public class PacientesBusinessLogic : GenericBusinessLogic<Pacientes>
    {
        public PacientesBusinessLogic(IUnitOfWork unitWork) : base(unitWork)
        {
        }

        public PacientesBusinessLogic(DataBaseSetting configuracionBD) : base(configuracionBD)
        {
        }
        ...
        public string CargarDatosPlantilla(MemoryStream memoryStream, bool modificaRegistros, string usuario, long empresaId)
        {
            ...
            try
            {
                ...
                Worksheet sheet = workbook.Worksheets.FirstOrDefault(x => x.Name == "PACIENTES");
                if (sheet == null)
                {
                    erroresExcel.Add("Error Hoja", new List<string> { $"Plantilla defectuosa. No contiene la hoja PACIENTES." });
                }
                else
                {
                    bool tieneDato = true;
                    for (int i = 1; tieneDato; i++)
                    {
                        ...
                        var logicaData = new GenericBusinessLogic<Pacientes>(this.UnitOfWork.Settings);

                        CellValue cellTipoIdentificacion = sheet.GetCellValue(sheet.Columns["A"].Index, i);
                        CellValue cellNumeroIdentificacion = sheet.GetCellValue(sheet.Columns["B"].Index, i);
                        CellValue cellPrimerNombre = sheet.GetCellValue(sheet.Columns["C"].Index, i);
                        CellValue cellSegundoNombre = sheet.GetCellValue(sheet.Columns["D"].Index, i);
                        CellValue cellPrimerApellido = sheet.GetCellValue(sheet.Columns["E"].Index, i);
                        CellValue cellSegundoApellido = sheet.GetCellValue(sheet.Columns["F"].Index, i);
                        CellValue cellFechaNacimiento = sheet.GetCellValue(sheet.Columns["G"].Index, i);
                        CellValue cellTelefono = sheet.GetCellValue(sheet.Columns["H"].Index, i);
                        CellValue cellDireccion = sheet.GetCellValue(sheet.Columns["I"].Index, i);
                        CellValue cellCorreo = sheet.GetCellValue(sheet.Columns["J"].Index, i);
                        ...
                        CellValue cellIdTipoIdentificacion = sheet.GetCellValue(sheet.Columns["U"].Index, i);
                        CellValue cellIdCiudad = sheet.GetCellValue(sheet.Columns["V"].Index, i);
                        CellValue cellIdGenero = sheet.GetCellValue(sheet.Columns["W"].Index, i);
                        CellValue cellIdTipoAfiliado = sheet.GetCellValue(sheet.Columns["X"].Index, i);
                        CellValue cellIdTipoRegimen = sheet.GetCellValue(sheet.Columns["Y"].Index, i);
                        CellValue cellIdTipoCategoria = sheet.GetCellValue(sheet.Columns["Z"].Index, i);

                        if (string.IsNullOrWhiteSpace(cellTipoIdentificacion.TextValue))
                        {
                            tieneDato = false;
                            break;
                        }
                        ...
                        {
                            Pacientes data = logicaData.FindById(x => x.TiposIdentificacionId == cellIdTipoIdentificacion.NumericValue && x.NumeroIdentificacion == cellNumeroIdentificacion.TextValue, false);
                            if (data == null)
                            {
                                data = new Pacientes();
                                data.Id = 0;
                            ...
                            data.CorreoElectronico = cellCorreo.TextValue;
                            if (!cellIdCiudad.IsEmpty)
                                data.CiudadesId = Convert.ToInt64(cellIdCiudad.TextValue);
                            if (!cellIdGenero.IsEmpty)
                                data.GenerosId = Convert.ToInt64(cellIdGenero.TextValue);
                            ...
                            if (!cellIdTipoAfiliado.IsEmpty)
                                data.TiposAfiliadosId = Convert.ToInt64(cellIdTipoAfiliado.TextValue);
                            if (!cellIdTipoRegimen.IsEmpty)
                                data.TiposRegimenesId = Convert.ToInt64(cellIdTipoRegimen.TextValue);
                            if (!cellIdTipoCategoria.IsEmpty)
                                data.CategoriasIngresosId = Convert.ToInt64(cellIdTipoCategoria.TextValue);
